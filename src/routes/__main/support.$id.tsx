...
import { ArrowLeft, ChevronUp, Send, SearchX } from 'lucide-react'
...
function RouteComponent() {
    ...
    const messagesQuery = useInfiniteQuery({
        ...
        queryFn: ({ pageParam }) => supportApi.listMessages(id, { cursor: pageParam, limit: MESSAGES_PAGE_SIZE }),
    ...
    const sendMutation = useMutation({
        mutationFn: (message: string) => supportApi.sendMessage(id, { message }),
        onSuccess: () => {
            setNewMessage('')
        ...
        <>
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                ...
                >
                    ...
                    <span className="text-sm font-medium">Back to Support</span>
                </Link>
                            ...
                            <div className="flex flex-col">
                                ...
                                <div className="flex items-center gap-2 mt-1">
                                    <Avatar size="sm">
                                        ...
                                        <AvatarFallback className="text-[10px]">{getInitials(ticket.createdBy.name)}</AvatarFallback>
                                    </Avatar>
                                    <div>
                                        <p className="text-sm font-medium text-foreground">{ticket.createdBy.name}</p>
                                        <p className="text-[11px] text-muted-foreground">{formatDateTime(ticket.createdAt)}</p>
                                    </div>
                                </div>
                            </div>
                            <div className="flex flex-col">
                        ...
                        {messages.map((msg, idx) => {
                            ...
                            return (
                                <div key={msg.id}>
                                    ...
                                    <div className={cn('flex gap-2.5 py-1.5', isAdmin ? 'flex-row-reverse' : 'flex-row')}>
                                        ...
                                        <div className={cn('flex flex-col gap-1 max-w-[78%]', isAdmin ? 'items-end' : 'items-start')}>
